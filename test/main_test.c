...
register_fd_ready_fn (memif_connection_t *c,
                         memif_fn *read_fn, memif_fn *write_fn, memif_fn *error_fn)
{
    c->read_fn = read_fn;
    c->write_fn = write_fn;
    c->error_fn = error_fn;
...
START_TEST (test_control_fd_handler)
{
    ...
    if ((err = memif_control_fd_handler (
                lm->timerfd, MEMIF_FD_EVENT_READ)) != MEMIF_ERR_SUCCESS)
        ck_assert_msg (err == MEMIF_ERR_NO_FILE,
    ...
    if ((err = memif_control_fd_handler (
                c->fd, MEMIF_FD_EVENT_WRITE)) != MEMIF_ERR_SUCCESS)
        ck_abort_msg ("err code: %u, err msg: %s", err, memif_strerror (err));

    ck_assert (ready_called & write_call);

    if ((err = memif_control_fd_handler (
                c->fd, MEMIF_FD_EVENT_ERROR)) != MEMIF_ERR_SUCCESS)
        ck_abort_msg ("err code: %u, err msg: %s", err, memif_strerror (err));
...
START_TEST (test_buffer_alloc)
{
    ...
    qid = 0;
    if ((err = memif_buffer_alloc (conn, qid, bufs, max_buf, &buf)) != MEMIF_ERR_SUCCESS)
        ck_abort_msg ("err code: %u, err msg: %s", err, memif_strerror (err));
    ...
    qid = 2;
    if ((err = memif_buffer_alloc (conn, qid, bufs, max_buf, &buf)) != MEMIF_ERR_SUCCESS)
        ck_assert_msg (err == MEMIF_ERR_QID,
                "err code: %u, err msg: %s", err, memif_strerror (err));
...
START_TEST (test_tx_burst)
{
    ...
    if ((err = memif_tx_burst (conn, qid, bufs, max_buf, &tx)) != MEMIF_ERR_SUCCESS)
        ck_abort_msg ("err code: %u, err msg: %s", err, memif_strerror (err));
...
START_TEST (test_rx_burst)
{
    ...
    if ((err = memif_rx_burst (conn, qid, bufs, max_buf, &rx)) != MEMIF_ERR_SUCCESS)
        ck_abort_msg ("err code: %u, err msg: %s", err, memif_strerror (err));
...
START_TEST (test_buffer_free)
{
    ...
    if ((err = memif_buffer_free (conn, qid, bufs, max_buf, &buf)) != MEMIF_ERR_SUCCESS)
        ck_abort_msg ("err code: %u, err msg: %s", err, memif_strerror (err));
